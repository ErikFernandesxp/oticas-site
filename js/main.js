...
document.addEventListener("DOMContentLoaded", () => {
  ...
  forms.forEach(form => {
    form.addEventListener("submit", function (e) {
      ...
      if (document.title.includes("Orçamentos")) {
        const produto = this.querySelector("select") ? this.querySelector("select").value : "";
        texto = `Olá, meu nome é *${nome}*.%0A` +
                `Meu WhatsApp: *${whats}*%0A` +
                (produto ? `Produto de interesse: *${produto}*%0A` : "") +
                `Mensagem: ${mensagem}`;
      } 
  ...
  // ===============================
  const imagens = document.querySelectorAll("main img, section img, article img, .galeria img, .grid img, .card img, .list img");
  if (imagens.length) {
    // cria overlay
    ...
    imagens.forEach(img => {
      ...
      img.addEventListener("click", () => {
        imgAmpliada.src = img.currentSrc || img.src;
